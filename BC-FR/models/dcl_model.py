...
class DCLModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        ################################################################################################################
        self.netA_A = networks.define_A(opt.input_nc, 1, opt.ngf, opt.netA, opt.normG, not opt.no_dropout,
                                        opt.init_type, opt.init_gain, opt.no_antialias, opt.no_antialias_up,
                                        self.gpu_ids, opt)
        self.netA_B = networks.define_A(opt.input_nc, 1, opt.ngf, opt.netA, opt.normG, not opt.no_dropout,
                                        opt.init_type, opt.init_gain, opt.no_antialias, opt.no_antialias_up,
                                        self.gpu_ids, opt)
        ################################################################################################################
        ...
        if self.isTrain:
            ...
            self.netPre2 = losses.VGG16().to(self.device)
            self.criterionSpatial = losses.SpatialCorrelativeLoss(opt.loss_mode, opt.patch_nums, opt.patch_size, opt.use_norm, opt.learned_attn, gpu_ids=self.gpu_ids, T=opt.T).to(self.device)
            self.criterionSpatial2 = losses.SpatialCorrelativeLoss(opt.loss_mode, opt.patch_nums, opt.patch_size, opt.use_norm, opt.learned_attn, gpu_ids=self.gpu_ids, T=opt.T).to(self.device)
            self.criterionStyle = losses.StyleLoss().to(self.device)
            self.criterionStyle2 = losses.StyleLoss().to(self.device)
            self.set_requires_grad([self.netPre], False)
            self.set_requires_grad([self.netPre2], False)
            self.criterioncc1 = torch.nn.L1Loss().to(self.device)
            self.criterioncc2 = torch.nn.L1Loss().to(self.device)
    ...
    def forward(self):
        ...
        self.merge_rec_A = self.mask_layer(self.real_A, self.fake_B, self.attn_real_A)
        self.attn_real_B = self.netA_B(self.real_B)
        self.merge_rec_B = self.mask_layer(self.real_B, self.fake_A, self.attn_real_B)
        ################################################################################################################
    ...
    def compute_G_loss(self):
        ...
        ################################################################################################################
        #self.norm_real_A = self.normalization((self.real_A + 1) * 0.5)
        #self.norm_fake_B = self.normalization((self.fake_B + 1) * 0.5)
        #self.norm_real_B = self.normalization((self.real_B + 1) * 0.5)
        #self.norm_fake_A = self.normalization((self.fake_A + 1) * 0.5)
        #self.norm_rec_A = self.normalization((self.rec_A + 1) * 0.5)
        #self.norm_rec_B = self.normalization((self.rec_B + 1) * 0.5)
        ################################################################################################################
        #self.loss_G_srec = self.Spatial_Loss(self.netPre, self.norm_real_A, self.norm_rec_A, None)
        #self.loss_G_srec2 = self.Spatial_Loss2(self.netPre2, self.norm_real_B, self.norm_rec_B, None)
        ################################################################################################################
        #self.loss_style2 = self.criterionStyle2(self.norm_real_B, self.norm_rec_B)
        #self.loss_style = self.criterionStyle(self.norm_real_A, self.norm_rec_A)
        ################################################################################################################
        ...
        if self.opt.lambda_NCE > 0.0:
            ...
            ############################################################################################################
            self.loss_NCE1_2 = self.calculate_NCE_loss1(self.fake_A, self.real_B) * self.opt.lambda_NCE
            self.loss_NCE2_2 = self.calculate_NCE_loss2(self.fake_B, self.real_A) * self.opt.lambda_NCE
            #self.loss_NCEid1 = self.calculate_NCE_lossid1(self.real_B,self.idB)
            #self.loss_NCEid2 = self.calculate_NCE_lossid2(self.real_A,self.idA)
            ############################################################################################################
        ...
        if self.opt.lambda_NCE > 0.0:
            ...
            loss_NCE_both = (self.loss_NCE1 + self.loss_NCE2) * 0.25 + (self.loss_idt_A + self.loss_idt_B) * 0.5

        else:
            loss_NCE_both = (self.loss_NCE1 + self.loss_NCE2) * 0.5

        ################################################################################################################
        self.loss_cycle_A = self.criterioncc1(self.merge_rec_A, self.real_A)
        self.loss_cycle_B = self.criterioncc2(self.merge_rec_B, self.real_B)
        ################################################################################################################
        ...
        ################################################################################################################
        img1r = self.attn_real_B.mul(255).byte()
        img1r_ = img1r.cpu().numpy().squeeze(0).transpose((1, 2, 0))

        img2r = self.attn_fake_A.detach().mul(255).byte()
        img2r_ = img2r.cpu().numpy().squeeze(0).transpose((1, 2, 0))

        self.attn_real_B_edge = cv2.Canny(img1r_, 0, 120)
        self.attn_fake_A_edge = cv2.Canny(img2r_, 0, 120)

        self.loss_edge_B_A = np.sqrt(np.sum(np.square(self.attn_real_B_edge - self.attn_fake_A_edge)))
        ################################################################################################################
        '''

        ################################################################################################################
        self.loss_G = (self.loss_G_A + self.loss_G_B) * 0.5 + loss_NCE_both \
                         + 0.25 * (self.loss_NCE1_2 + self.loss_NCE2_2) \
                         + 0.1 * (self.loss_cycle_A + self.loss_cycle_B)
                         # + 0.05 * (self.loss_edge_A_B + self.loss_edge_B_A)
                         #+ 0.05 * (self.loss_style + self.loss_style2)
                         #+ 0.05 * (self.loss_G_srec + self.loss_G_srec2) \
        ################################################################################################################
        return self.loss_G
    ...
    ####################################################################################################################
    def calculate_NCE_lossid1(self, src, tgt):
        n_layers = len(self.nce_layers)
        feat_q = self.netG_B(tgt, self.nce_layers, encode_only=True)
        feat_k = self.netG_A(src, self.nce_layers, encode_only=True)
        feat_k_pool, sample_ids = self.netF2(feat_k, self.opt.num_patches, None)
    ...
    def calculate_NCE_lossid2(self, src, tgt):
        n_layers = len(self.nce_layers)
        feat_q = self.netG_A(tgt, self.nce_layers, encode_only=True)
        feat_k = self.netG_B(src, self.nce_layers, encode_only=True)
        feat_k_pool, sample_ids = self.netF1(feat_k, self.opt.num_patches, None)
    ...
    def Spatial_Loss2(self, net, src, tgt, other=None):
        """given the source and target images to calculate the spatial similarity and dissimilarity loss"""
        n_layers = len(self.attn_layers)
        feats_src = net(src, self.attn_layers, encode_only=True)
        feats_tgt = net(tgt, self.attn_layers, encode_only=True)
        ...
        total_loss = 0.0
        for i, (feat_src, feat_tgt, feat_oth) in enumerate(zip(feats_src, feats_tgt, feats_oth)):
            loss = self.criterionSpatial2.loss(feat_src, feat_tgt, feat_oth, i)
            total_loss += loss.mean()

        if not self.criterionSpatial2.conv_init:
            self.criterionSpatial2.update_init_()
